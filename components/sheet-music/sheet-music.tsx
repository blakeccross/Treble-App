import React from "react";
import { View, ScrollView, StyleSheet } from "react-native";

type NoteType = "whole" | "half" | "quarter";
type Pitch = "d4" | "e4" | "f4" | "g4" | "a5" | "b5" | "c5" | "d5" | "e5" | "f5" | "g5";

interface NoteProps {
  type: NoteType;
  pitch: Pitch;
}

const MusicalStaff = () => {
  const StaffLines = () => (
    <View style={styles.staffLinesContainer}>
      {[0, 8, 16, 24, 32].map((top, index) => (
        <View key={index} style={[styles.staffLine, { top }]} />
      ))}
    </View>
  );

  const Bar = ({ type = "single", isEnd = false }: { type?: "single" | "double"; isEnd?: boolean }) => (
    <View style={[styles.barContainer, { width: type === "double" || isEnd ? 4 : 2 }]}>
      <View style={styles.barLine} />
      {(type === "double" || isEnd) && <View style={[styles.barLine, { left: 0, width: isEnd ? 4 : 1 }]} />}
    </View>
  );

  const Note = ({ type, pitch }: NoteProps) => {
    const getNotePosition = (pitch: Pitch): number => {
      const positions: Record<Pitch, number> = {
        d4: 32,
        e4: 28,
        f4: 24,
        g4: 20,
        a5: 16,
        b5: 12,
        c5: 8,
        d5: 4,
        e5: 0,
        f5: -4,
        g5: -8,
      };
      return positions[pitch] || 0;
    };

    const noteTop = getNotePosition(pitch);
    const needsHighStem = ["d5", "e5", "f5", "g5"].includes(pitch);

    const headSpecificStyle = type === "whole" ? styles.wholeNote : type === "half" ? styles.halfNote : styles.quarterNote;

    return (
      <View style={[styles.noteContainer, { top: noteTop }]}>
        <View style={[styles.noteHead, headSpecificStyle]} />
        {(type === "half" || type === "quarter") && <View style={[styles.stem, { top: needsHighStem ? 4 : -24 }]} />}
      </View>
    );
  };

  return (
    <View style={styles.container}>
      <ScrollView horizontal>
        <View style={styles.staffContainer}>
          <StaffLines />
          <View style={styles.notesContainer}>
            <Note type="half" pitch="e4" />
            <Note type="quarter" pitch="f4" />
            <Note type="quarter" pitch="g4" />
            <Bar />
            <Note type="quarter" pitch="a5" />
            <Note type="half" pitch="b5" />
            <Note type="quarter" pitch="c5" />
            <Bar type="double" />
            <Note type="quarter" pitch="c5" />
            <Note type="half" pitch="a5" />
            <Note type="quarter" pitch="e5" />
            <Bar />
            <Note type="quarter" pitch="c5" />
            <Note type="quarter" pitch="e5" />
            <Note type="quarter" pitch="f5" />
            <Note type="quarter" pitch="g4" />
            <Bar />
            <Note type="half" pitch="d5" />
            <Note type="half" pitch="a5" />
            <Bar />
            <Note type="whole" pitch="d4" />
            <Bar />
            <Bar isEnd={true} />
          </View>
        </View>
      </ScrollView>
    </View>
  );
};

const styles = StyleSheet.create({
  container: {
    padding: 20,
    backgroundColor: "#fff",
  },
  staffContainer: {
    minHeight: 120,
  },
  staffLinesContainer: {
    position: "absolute",
    width: "100%",
    height: 80,
    top: 0,
  },
  staffLine: {
    position: "absolute",
    width: "100%",
    height: 1,
    backgroundColor: "#999",
  },
  notesContainer: {
    flexDirection: "row",
    alignItems: "flex-start",
  },
  barContainer: {
    height: 32,
    marginHorizontal: 12,
    marginBottom: 48,
  },
  barLine: {
    position: "absolute",
    width: 1,
    height: 32,
    backgroundColor: "black",
    right: 0,
  },
  noteContainer: {
    width: 12,
    height: 32,
    marginHorizontal: 12,
    marginBottom: 48,
  },
  noteHead: {
    height: 8,
    width: 12,
  },
  wholeNote: {
    borderWidth: 1,
    borderLeftWidth: 4,
    borderRightWidth: 4,
    borderRadius: 50,
    backgroundColor: "transparent",
  },
  halfNote: {
    borderWidth: 1,
    borderTopWidth: 2.18,
    borderBottomWidth: 2.18,
    borderRadius: 50,
    transform: [{ rotate: "-15deg" }],
    backgroundColor: "transparent",
  },
  quarterNote: {
    backgroundColor: "black",
    borderRadius: 50,
    transform: [{ rotate: "-15deg" }],
  },
  stem: {
    position: "absolute",
    width: 1,
    height: 28,
    backgroundColor: "black",
    right: 0,
  },
});

export default MusicalStaff;
