...
export default class ReactNativeSVGContext extends SVGContext {
  ...
  constructor(fontPack, { width, height, color }) {
  ...
  applyAttributes(element, attributes) {
    ...
    if (this.color) {
      // Use the stored color for fill and stroke
      element.props.fill = this.color;
      element.props.stroke = this.color;
    }
  ...
  fillText(text, x, y) {
    ...
    return this; // Ensure that fillText returns 'this'
  }
  ...
  createReactElement(element) {
    ...
    return React.createElement(svgClass[element.svgElementType], element.props, children);
  }
